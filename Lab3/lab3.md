...
在之后的函数调用中，`ra`寄存器理应保存了真实的上下文环境，也即函数的返回地址，由于之前进程在`dummy()`函数的死循环中无限循环，而在`dummy()`中先触发了中断，然后进入线程切换的程序，因此返回之后应当重新回到解决中断的函数中，即`_traps`中的`call trap_handler`处，也正是此处设置了`ra`寄存器。

![](https://raw.githubusercontent.com/FanBB2333/picBed/main/img/20211128220909.png)
